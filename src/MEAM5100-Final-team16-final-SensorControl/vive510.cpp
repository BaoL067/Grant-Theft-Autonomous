...
Vive510::Vive510(int pin, const String& name) {
  m_pin = pin;
  m_name = name;
}
...
uint16_t Vive510::xCoord_filtered(int numSamples){
  uint16_t* x = new uint16_t[numSamples];
  int i;
  ...
  delete[] x;
  return medianFilter(x, numSamples);
}

uint16_t Vive510::yCoord_filtered(int numSamples){
  uint16_t* y = new uint16_t[numSamples];
  int i;
  for (i = 0 ; i<numSamples; i++){
    y[i] = yCoord();
    delay(30);
  ...
  delete[] y;
  return medianFilter(y, numSamples);
}
...
void Vive510::processPulse() {
  if (m_lastFalling != m_usFalling) {
    int pulsewidth = m_usFalling-m_usRising-3;
...
uint16_t Vive510::medianFilter(uint16_t* values, int numSamples) {
    // find the median
